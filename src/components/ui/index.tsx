...
/**
 * Thin wrapper around `Pagination` that takes a `usePagedQuery`/`useClientPage`
 * result directly, instead of every call site re-typing the same 7-prop
 * mapping (`rowCount={page.rows.length}`, `onPrev={page.goPrev}`, ...). New
 * paginated lists should use this; the ~19 existing hand-spread call sites
 * are left alone for a separate cleanup pass.
 */
export function PagedFooter({
  ...
  return (
    <Pagination
      ...
      onPrev={paged.goPrev}
      onNext={paged.goNext}
      loading={loading ?? paged.isLoading}
    />
  );
}

/**
 * Side panel sliding in from the right — same backdrop/animation pattern as
 * `Modal`, for content too tall/persistent to be a centered dialog (e.g. the
 * Ficha do Cliente opened from the POS rail).
 */
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <motion.div
            ...
            className="relative h-full w-full max-w-lg bg-surface rounded-l-2xl shadow-xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between px-5 py-4 border-b border-outline/40">
              <div>
            ...
            {footer && (
              <div className="flex justify-end gap-2 px-5 py-4 border-t border-outline/40">
                {footer}
              </div>
            )}
          </motion.div>
